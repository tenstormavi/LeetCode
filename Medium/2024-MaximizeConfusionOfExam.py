...
# Exactly Same solution as 424
class Solution:
    def maxConsecutiveAnswers(self, answerKey: str, k: int) -> int:
        # Approach: length (end - start + 1) - max_occurance_char <= k --> shift r else l
        max_len = l = 0
        count_map = {}
        for r in range(len(answerKey)):
            count_map[answerKey[r]] = 1 + count_map.get(answerKey[r], 0)
            while (r - l + 1) - max(count_map.values()) > k:
                count_map[answerKey[l]] -= 1
                l += 1
