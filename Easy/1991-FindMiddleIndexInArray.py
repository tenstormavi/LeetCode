...
class Solution:
    def findMiddleIndex(self, nums: List[int]) -> int:
        # Approach: Calculate total sum of the array and start iterating.
        # Remove the element from the total sum as we proceed and check if sum is equal or not.
        lsum = 0
        rsum = sum(nums)

        for idx, num in enumerate(nums):
            # Remove the element from the total sum and check
            rsum -= num
            if lsum == rsum:
                return idx

            # Increase the cur sum
            lsum += num
        return -1
