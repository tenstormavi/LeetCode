...
is still the winner, and you should also return true. You may assume that both players are playing optimally.
...
class Solution:
    def predictTheWinner(self, nums: List[int]) -> bool:
        ...
        # Solution 1: Only recursion (Brute force)
        def score(l, r):
            if l > r:
                return 0
            score_taking_left = nums[l] - score(l+1, r)
            score_taking_right = nums[r] - score(l, r-1)
            return max(score_taking_left, score_taking_right)
        return score(0, n-1) >= 0

        # Solution 1: Recursion with DP
