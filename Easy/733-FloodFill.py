...
class Solution:
    def floodFill(self, image: List[List[int]], sr: int, sc: int, color: int) -> List[List[int]]:
        def fill(grid, i, j, row, col, present_color, color):
            if i < 0 or i >= row or j < 0 or j >= col or grid[i][j] != present_color:
                return
            grid[i][j] = color

            fill(grid, i + 1, j, row, col, present_color, color)
            fill(grid, i - 1, j, row, col, present_color, color)
            fill(grid, i, j + 1, row, col, present_color, color)
            fill(grid, i, j - 1, row, col, present_color, color)
        ...
        fill(image, sr, sc, row, col, present_color, color)
        return image
