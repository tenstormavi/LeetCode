...
class Solution:
    def jump(self, nums: List[int]) -> int:
        # If it is guranteed that we can reach last index then only this solution will work.
        # Greedy - O(n) - BFS
        res = 0
        l = 0  # First level
        r = 0  # First level
        while r < len(nums) - 1:
            farthest = 0
            for i in range(l, r + 1):  # +1 because r inclusive
                farthest = max(farthest, i + nums[i])
            l = r + 1  # new level start just after the previous level so +1
            r = farthest
            res += 1
        return res
