...
class FreqStack:
    ...
    def __init__(self):
        self.freq = defaultdict(int)  # defaultdict(<class 'int'>, {1: 2})
        self.map = defaultdict(list)  # defaultdict(<class 'list'>, {1: [2]})
        self.max_freq = 0
    ...
    def pop(self) -> int:
        val = self.map[self.max_freq].pop()
        if not self.map[self.max_freq]:
            self.max_freq -= 1
        self.freq[val] -= 1
        return val
