...
and going backward) for which the stock price was less than or equal to the price of that day.
    ...
    today is 2, then the span of today is 4 because starting from today, the price of the stock was less
    than or equal 2 for 4 consecutive days. Also, if the prices of the stock in the last four days is
    [7,34,1,2] and the price of the stock today is 8, then the span of today is 3 because starting from
...
class StockSpanner:
    ...
    def next(self, price: int) -> int:
        # Approach - if y > z; and x > y then x is obviously greater than z by default
        # Take the result count of the popped price and add the count with the current price.

        result = 1  # Count for itself
